...
def validate_and_update_data(data, user_id, session_id):
    ...
    """
    prefix = "uploaded_"
    identifier_types = ['llpin', 'cin']
    entity_types = ['company', 'llp',]
    client_msme_status_types = ['already_registered', 'not_registered','unknown']
    client_z_altman_types = ['trading', 'manufacturing']
    for index, row in enumerate(data, start=1):
        # Add prefix to all keys
        prefixed_row = {
    f"uploaded_{key}": value for key, value in row.items()
} | {
    f"unmodified_{key}": value for key, value in row.items()
}
        prefixed_row = sanitize_record(prefixed_row)
        missing_fields = []
        ...
        if missing_fields:
            raise ValueError(f"Name is mandatory. Please make sure your Excel file contains values in the column")
        identifier_type = prefixed_row.get('uploaded_identifier_type','')
        if identifier_type:
            if identifier_type.lower() not in identifier_types:
                raise ValueError(f"Invalid Identifier Type. Please make sure identifiers are either LLPIN or CIN")
        entity_type = prefixed_row.get('uploaded_entity_type', '')
        if entity_type:
            if entity_type.lower() not in entity_types:
                raise ValueError(
                    f"Invalid Entity Type. Please make sure entity types are either Company or LLP")
        client_msme_status = prefixed_row.get('uploaded_client_msme_status', '')
        if client_msme_status:
            if client_msme_status.lower() not in client_msme_status_types:
                raise ValueError(
                    f"Invalid Client MSME Status Type. Please make sure MSME STATUS are either ALREADY_REGISTERED or NOT_REGISTERED or UNKNOWN")

        client_z_altman_type = prefixed_row.get('uploaded_client_z_altman_type', '')
        if client_z_altman_type:
            if client_z_altman_type.lower() not in client_z_altman_types:
                raise ValueError(
                    f"Invalid Client Z ALTMAN Type. Please make sure Z ALTMAN Type is either TRADING or MANUFACTURING"
                )
...
async def process_excel_file(file_contents, client_id: str, user_id: str, user_email: str, session) -> Dict:
    try:
        ...
        print(sheet_data)
        is_inserted = await insert_dynamic_data("upload_supplier_data", sheet_data, session)
...
async def get_session_supplier(sess_id, page_no, rows_per_page, final_validation_status, session) -> Dict:
    try:
        ...
        select_column = ["id", "uploaded_name", "uploaded_identifier", "uploaded_entity_type",
                        "uploaded_identifier_type", "uploaded_address", "uploaded_client_msme_status",
                        "uploaded_client_onboarding_date", "uploaded_client_z_altman_type",
                        "ens_id", "session_id", "validation_status", "suggested_name", "suggested_identifier",
                        "suggested_entity_type", "suggested_identifier_type",
                         "orbis_matched_status", "final_status", "final_validation_status", "match_percentage", "duplicate_in_session", "uploaded_external_vendor_id"]
        ...
        # table_class = Base.metadata.tables.get("upload_supplier_data")
...
async def update_suggestions_bulk(payload, session) -> Dict:
    try:
        ...
        if table_class is None:
            raise HTTPException(
                status_code=404, 
                detail="Table 'upload_supplier_data' does not exist in the database schema."
            )
        ...
        logger.info(f"Rejected Rows with MATCH status: {rejected_rows.rowcount}")
        ...
        reject_or_accept_others_query = (
            ...
            .values(
                ...
                cin_id=table_class.c.suggested_cin_id,
                bid=table_class.c.suggested_cin_id,
                final_status=final_response,
                identifier=table_class.c.suggested_identifier,
                identifier_type=table_class.c.suggested_identifier_type,
                entity_type=table_class.c.suggested_entity_type,
                ...
                uploaded_client_msme_status =table_class.c.uploaded_client_msme_status,
                uploaded_client_z_altman_type = table_class.c.uploaded_client_z_altman_type,
            )
        )
        ...
        if accepted_ensid:
            accept_query = (
                ...
                .values(
                    ...
                    uploaded_client_onboarding_date=table_class.c.uploaded_client_onboarding_date,
                    uploaded_client_msme_status=table_class.c.uploaded_client_msme_status,
                    uploaded_client_z_altman_type=table_class.c.uploaded_client_msme_status,
                )
            )
